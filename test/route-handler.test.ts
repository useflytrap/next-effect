...
Deno.test("route handler > responses > bytes", async () => {
  const bytesGET = testRouteHandler(
    Effect.sync(() => new Uint8Array([1, 2, 3])),
  );

  const bytesResult = await bytesGET(
    new NextRequest("https://www.example.com"),
  );
  assertEquals(bytesResult.status, 200);
  assertEquals(
    bytesResult.headers.get("Content-Type"),
    "application/octet-stream",
  );
  assertEquals(
    await bytesResult.arrayBuffer(),
