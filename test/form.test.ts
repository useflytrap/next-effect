...
const createApiKeySuccess = async (_prevState: any, formFields: any) =>
  Effect.sync(() => {
    return {
      success: true,
...
const createApiKeyDefect = async (_prevState: any, _formFields: any) =>
  Effect.sync(() => {
...
Deno.test("server actions > forms > errors", async () => {
  const createApiKeyAction = makeFormHandler({
    ...
    errors: {
      invalidFormData: (errors) => ({
        success: false,
        message: "Invalid payload",
        errors,
      }),
      unexpected: () =>
        ApiKeyState.make({ success: false, message: "Internal server error" }),
    },
  });
  ...
  const resultDefect = await createApiKeyActionDefect(
    { success: false },
    mockFormDataDefect,
  );
...
Deno.test("server actions > forms > custom layer", async () => {
  ...
  const createApiKeyAction = makeFormHandler({
    state: ApiKeyState,
    fields: ApiKeyFormSchema,
    action: async (prevState, formFields) =>
      Effect.gen(function* () {
...
Deno.test("server actions > forms > can use next service", async () => {
  ...
  const result = await createApiKeyForm(
